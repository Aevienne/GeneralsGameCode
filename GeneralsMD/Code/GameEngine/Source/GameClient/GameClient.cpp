...
void GameClient::update()
{
  ...
  if (TheInGameUI->isCameraTrackingDrawable())
  {
	...
	if (!freezeTime)
	{
		...
		for (Drawable* d = firstDrawable(); d; d = d->getNextDrawable()) draws.push_back(d);
		if (TheJobSystem && TheJobSystem->isActive() && draws.size() > 64) {
			std::vector<Drawable*> toDestroy; std::mutex destroyMutex;
			TheJobSystem->parallelFor((int)draws.size(), [&](int i){
				Drawable* draw = draws[i];
				if (draw->getExpirationDate() != 0 && TheGameLogic->getFrame() >= draw->getExpirationDate()) {
					std::lock_guard<std::mutex> lk(destroyMutex);
					toDestroy.push_back(draw);
					return;
				}
#if ENABLE_CONFIGURABLE_SHROUD
				if (TheGlobalData->m_shroudOn)
			...
			for (auto* d : toDestroy) destroyDrawable(d);
		} else {
